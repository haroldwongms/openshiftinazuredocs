...
**OpenShift Origin with a single master**
...
Depending on the branch, the Azure Resource Manager templates for OpenShift Container Platform and OpenShift Origina may provide input parameters for enabling metrics and logging. 

The OpenShift Container Platform Marketplace offer automatically installs metrics and logging so no further action is needed.

If metrics and / or logging was not enabled for whatever reason, you can easily execute the appropriate Ansible playbook to enable the desired feature. 
...
2. Edit the /etc/ansible/hosts file and add the following lines after the Identity Provider Section (# Enable HTPasswdPasswordIdentityProvider). If these lines are already present, do not add them again.
